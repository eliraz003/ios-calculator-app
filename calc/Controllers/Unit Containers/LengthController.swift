...
var lengthUnits = UnitContainer({ add, reference in
    ...
    add(.HideInResultRendering, Length(name: Length.Inch, symbol: "IN", rate: 25.4))
    add(.HideInResultRendering, Length(name: Length.Foot, symbol: "FT", rate: 304.8))
    ...
    add(.ShowInResultRendering, Length.LengthRendered(name: "FT'IN", syntax:"FT\" IN'", symbol:"FT", fallback: reference.getWithName(Length.Foot)))
    
    add(.ShowInResultRendering, Length.LengthRendered(name: "YRD FT'", syntax:"YRD FT\"", symbol:"YRD FT", fallback: reference.getWithName(Length.Yard)))
})
